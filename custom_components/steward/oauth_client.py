...
its client_id URL using exact string comparison. Claude Code registers a
loopback redirect with no port and then listens on a random one, so the two
never match and Home Assistant answers "Invalid redirect URI".

Serving our own document sidesteps that without a separate authorization
server: it lists a small set of fixed loopback ports, the client pins its
callback to one of them, and Home Assistant's own login flow does the rest.
...
# Ports a client may pin its loopback callback to. Both host spellings are
